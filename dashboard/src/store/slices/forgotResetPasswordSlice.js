...
export const forgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const response = await axios.post(
      "https://freelance-portfolio-production-8edc.up.railway.app/api/v1/user/password/forgot",
      { email },
  ...
  (token, password, confirmPassword) => async (dispatch) => {
    try {
      ...
      const response = await axios.put(
        ` https://freelance-portfolio-production-8edc.up.railway.app/api/v1/user/password/reset/${token}`,
